...
app.get('/product/:id',async(req,res)=>{
    try{
        const{id}=req.params;
        if(!id){
            return res.status(400).json({messege:'product is missing'})
        }
        const{token}=req.headers;
            ...
            if(userEmailFromToken.email){
                ...
                return res.status(200).json({messege:'success',product});
...
app.patch("/product/edit/:id", async (req, res) => {
  ...
  app.delete('/product/delete/:id',async(req,res)=>{
    const {id}=req.params;
    if(!id){
        return res.status(400).json({messege:'product id not found'}
    ...
    try{
        ...
        res.status(200).json({messege:"product delete successfully",product:deleteProduct})
    }catch (error) {
        res.status(400).json({
          message: "Internal Server Error Occured While Updating Product",
        });
